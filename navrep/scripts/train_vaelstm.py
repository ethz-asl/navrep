...
from navrep.tools.test_worldmodel import vaelstm_worldmodel_error
...
if __name__ == "__main__":
    ...
    if args.environment == "navreptrain":
        ...
        log_path = os.path.expanduser("~/navrep/logs/W/navreptrain_vaelstm_train_log_{}.csv".format(START_TIME))
        checkpoint_path = os.path.expanduser("~/navrep/models/W/navreptrainvaelstm")
    else:
    ...
    # create model
    model = VAELSTM(mconf)
    print("VAELSTM trainable params: {}".format(
    ...
    for epoch in range(max_epochs):
        ...
        for it, (x, a, y, x_rs, y_rs, dones) in pbar:
            ...
            h0 = torch.randn(mconf.lstm.n_layer, this_batch_size, mconf.lstm.h_size)
            c0 = torch.randn(mconf.lstm.n_layer, this_batch_size, mconf.lstm.h_size)
            ...
            with torch.set_grad_enabled(is_train):
                y_pred, y_rs_pred, loss = model(x, x_rs, a, dones, [h0, c0], targets=(y, y_rs))
                loss = (
                    loss.mean()
            ...
            if is_train:
                ...
                if global_step == 1 or global_step % 1000 == 0:
                    ...
                    plt.savefig(os.path.expanduser(
                        "~/tmp_navrep/vaelstm_step{:07}.png").format(global_step))

        lidar_e = None
        state_e = None
        if epoch % 20 == 0:
            lidar_e, state_e = vaelstm_worldmodel_error(model, dataset_dir, device)
