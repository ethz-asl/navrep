...
from navrep.models.gpt import GPTConfig, save_checkpoint, set_seed
from navrep.tools.wdataset import WorldModelDataset
from navrep.tools.test_worldmodel import gpt1d_worldmodel_error
...
if __name__ == "__main__":
    ...
    if args.environment == "navreptrain":
        ...
        log_path = os.path.expanduser("~/navrep/logs/W/navreptrain_gpt1d_train_log_{}.csv".format(START_TIME))
        checkpoint_path = os.path.expanduser("~/navrep/models/W/navreptraingpt1d")
    else:
            ...
            if is_train:
                ...
                if global_step == 1 or global_step % 1000 == 0:
                    ...
                    for i, (ax0, ax1, ax2) in enumerate(axes.T):
                        ax0.plot(np.squeeze(x.cpu()[0, 5 + i]))
                        ax1.plot(np.squeeze(y.cpu()[0, 5 + i]))
                        ax2.plot(np.squeeze(y_pred.detach().cpu()[0, 5 + i]))
                        ax2.set_xlabel("Done {}".format(dones.cpu()[0, 5 + 1]))
                    plt.savefig(os.path.expanduser(
                        "~/tmp_navrep/gpt1d_step{:07}.png").format(global_step))

        lidar_e = None
        state_e = None
        if epoch % 20 == 0:
            lidar_e, state_e = gpt1d_worldmodel_error(model, dataset_dir, device)
