...
def test_navrep_is_clean():
    if os.path.isdir(os.path.expanduser("~/navrep/models")) or \
        ...
        if 'OVERWRITE_NAVREP' in os.environ and os.environ['OVERWRITE_NAVREP'] == '1':
            print("WARNING: OVERWRITE_NAVREP is set to 1, will overwrite files in ~/navrep dir")
        else:
            print("""ERROR: running tests will overwrite data in the ~/navrep folder!
